...
interface DataType extends MovieCategory {
	key: string;
}
...
const DEFAULT_MOVIE_CATEGORY_VALUE: DataType = {
	key: "",
	id: undefined,
	name: "",
};
...
export default function MovieCategoryListPage() {
	...
	const getMovieCategoryList = async () => {
		const dataList = ((await apiLibraryMovieCategoryList()).page.list.map((item, index) => ({
			...item,
			key: index.toString() || "",
		})) || []) as DataType[];
		...
		try {
			...
			if (index > -1) {
				...
				if (newData[index].id === undefined) {
					await apiLibraryMovieCategorySave({
						name: newData[index].name,
					});
				} else {
					await apiLibraryMovieCategoryUpdate({
						id: newData[index].id,
						name: newData[index].name,
					});
				}
