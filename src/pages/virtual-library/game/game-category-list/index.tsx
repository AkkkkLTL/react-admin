...
interface DataType extends GameCategory {
	key: string;
}
...
const DEFAULT_GAME_CATEGORY_VALUE: DataType = {
	key: "",
	id: undefined,
	name: "",
};

export default function GameCategoryListPage() {
	...
	const getGameCategoryList = async () => {
		const dataList = ((await apiLibraryGameCategoryList()).page.list.map((item, index) => ({
			...item,
			key: index.toString() || "",
		})) || []) as DataType[];
		...
		try {
			...
			if (index > -1) {
				...
				if (newData[index].id === undefined) {
					await apiLibraryGameCategorySave({
						name: newData[index].name,
					});
				} else {
					await apiLibraryGameCategoryUpdate(newData[index]);
				}
				setRefresh(Date.now());
			} else {
