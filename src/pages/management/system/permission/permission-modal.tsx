...
import type { Menu, MenuTree, Permission } from "@/types/entity";
...
export type PermissionModalProps = {
	...
	title: string;
	show: boolean;
	onOk: (values: MenuTree) => void;
	onCancel: VoidFunction;
};

export default function PermissionModal({ formValue, title, show, onOk, onCancel }: PermissionModalProps) {
	const form = useForm<MenuTree>({
		defaultValues: formValue,
	});

	const permissions: MenuTree[] = [];
	const [compOptions, setCompOptions] = useState(PAGE_SELECT_OPTIONS);

	const getParentNameById = useCallback((parentId: string, data: MenuTree[] | undefined = permissions) => {
		let name = "";
	...
	const onSubmit = (values: MenuTree) => {
		onOk(values);
	};
