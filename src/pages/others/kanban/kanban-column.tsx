...
import { type CSSProperties, MouseEventHandler, memo, type PropsWithChildren, useRef, useState } from "react";
...
export default memo(function KanbanColumn({
	...
	const handleClickOutside = (event: MouseEvent) => {
		if (addTaskInputRef.current && !addTaskInputRef.current.contains(event.target as Node)) {
			...
			if (renameTaskInputRef.current && !renameTaskInputRef.current.contains(event.target as Node)) {
				const renameTaskInputVal = renameTaskInputRef.current.value;
				if (renameTaskInputVal) {
					renameColumn({ ...column, title: renameTaskInputVal });
				}
	...
	return (
		<div ref={ref} style={style} data-shadow={isDragging || undefined}>
			<header className="mb-4 flex select-none items-center justify-between text-base font-semibold">
							...
							if (item && "key" in item && "label" in item) {
								return (
									<DropdownMenuItem
										...
										onClick={(e) => {
											e.stopPropagation();
											handleMenuItemClick({ key: item.key });
										}}
									>
										{item.label}
									</DropdownMenuItem>
