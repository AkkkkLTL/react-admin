...
export function getVariant(variant = "slideInUp"): MotionProps["variants"] {
	return {
		// Slide
		...
		zoomInLeft: varZoom({ distance: 80 }).inLeft,
		zoomInRight: varZoom({ distance: 80 }).inRight,
		zoomOut: varZoom().out,
	...
	}[variant] as MotionProps["variants"];
}
