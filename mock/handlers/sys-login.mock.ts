...
import { SysLoginApi, type SysLoginRes } from "@/api/services/sys-login.service";
...
const mockSysLogin = http.post(`${apiPrefix}${SysLoginApi.Login}`, async () => {
	return HttpResponse.json<Result & SysLoginRes>(
		{
			msg: "success",
			code: ResultEnum.SUCCESS,
			expire: faker.number.int({ min: 60 * 60 * 1, max: 60 * 60 * 12 }),
			token: faker.string.fromCharacters("abcdefghijklmnopqrstuvwxyz0123456789", 32),
		},
		{
			status: 200,
		},
